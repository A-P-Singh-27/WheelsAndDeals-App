...
function MyListing() {
    ...
    const GetUserCarListing = async (req, res) => {
        ...
        try {
            const response = await fetch(`https://wheelsanddeals.onrender.com/api/cars/getusercarlisting?createdby=${user?.primaryEmailAddress?.emailAddress}`, {
                method: 'GET',
                headers: {
    ...
    async function deletecarlist(id) {
        try {
            const response = await fetch(`https://wheelsanddeals.onrender.com/api/cars/deletecarlist?id=${id}`, {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
                },
